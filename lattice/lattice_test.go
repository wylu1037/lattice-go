...
func TestLattice_TransferWaitReceipt(t *testing.T) {
	...
	hash, receipt, err := lattice.TransferWaitReceipt(context.Background(), "zltc_S5KXbs6gFkEpSnfNpBg3DvZHnB9aasa6Q", "0x10", NewBackOffRetryStrategy(10, time.Second))
...
func TestLattice_DeployContractWaitReceipt(t *testing.T) {
	...
	hash, receipt, err := lattice.DeployContractWaitReceipt(context.Background(), data, "0x", DefaultFixedRetryStrategy())
	assert.NoError(t, err)
	t.Log(hash.String())
	...
	hash, receipt, err = lattice.CallContractWaitReceipt(context.Background(), builtin.VoteBuiltinContract.Address, approveData, "0x", DefaultFixedRetryStrategy())
	assert.NoError(t, err)
	t.Log(hash.String())
...
func TestLattice_CallContractWaitReceipt(t *testing.T) {
	...
	assert.NoError(t, err)
	hash, receipt, err := lattice.CallContractWaitReceipt(context.Background(), "zltc_a5jerZSQ2UNMhDbUkHaYuVbiBMxkZWETj", data, "0x", DefaultFixedRetryStrategy())
	assert.NoError(t, err)
	t.Log(hash.String())
	r, err := json.Marshal(receipt)
	assert.NoError(t, err)
...
func TestLattice_CallContract(t *testing.T) {
	...
	hash, receipt, err := lattice.CallContractWaitReceipt(context.Background(), builtin.VoteBuiltinContract.Address, data, "0x", DefaultFixedRetryStrategy())
	assert.NoError(t, err)
	t.Log(hash.String())
	r, err := json.Marshal(receipt)
	assert.NoError(t, err)
