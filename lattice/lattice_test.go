...
func TestLattice_DeployContractWaitReceipt(t *testing.T) {
	...
	hash, receipt, err := latticeClient.DeployContractWaitReceipt(context.Background(), chainId, data, "0x", 0, 0, DefaultFixedRetryStrategy())
	assert.NoError(t, err)
	t.Log(hash.String())
	...
	hash, receipt, err = latticeClient.CallContractWaitReceipt(context.Background(), chainId, builtin.VoteBuiltinContract.Address, approveData, "0x", 0, 0, DefaultFixedRetryStrategy())
	assert.NoError(t, err)
	t.Log(hash.String())
